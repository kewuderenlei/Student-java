...
<html lang="en">
  ...
  <body class="app sidebar-mini rtl">
                ...
                <tbody>
                    <c:forEach items="${courseList}" var="course" varStatus="status">
                        <tr name="${course}">
                            ...
                            <td>${course.cTerm}</td>
                            <td>${course.cName}</td>
                            <td>${course.cTeaName}</td>
                            <td>${course.cHours}</td>
                            <td>${course.cCredit}</td>
                            <td>${course.cType}</td>
                            <td>
                                <c:choose>
                                    <c:when test="${course.cStatus == '已选'}">
                                        <a class="btn btn-danger" href="javascript:deleteCourse('<%=basePath%>', '${course.cNo}', '${course.cTeacher}');">退选</a>
                                    </c:when>
                                    <c:otherwise>
                                        <a class="btn btn-primary" href="javascript:chooseCourse('<%=basePath%>', '${course.cNo}', '${course.cTeacher}');">选课</a>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                        </tr>
    ...
    <script type="text/javascript">
        function chooseCourse(basePath, courseId, teaId) {
        ...
        function deleteCourse(basePath, courseId, teaId) {
            var id = document.getElementById("courseId");
            id.value = courseId;
            $("#teaId").val(teaId);
            var chooseCourseForm = document.getElementById("chooseCourseForm");
